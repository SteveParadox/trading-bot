...
export function App() {
  ...
  const applySnapshot = useCallback((snapshot: LiveSnapshot) => {
    setData((current) => ({ ...current, status: snapshot.status, positions: snapshot.positions, equity: mergeEquity(current.equity, snapshot.equity_curve), performance: snapshot.performance, config: snapshot.config, signals: snapshot.recent_signals, trades: filtersRef.current.instrument || filtersRef.current.outcome ? current.trades : snapshot.recent_trades }));
  ...
  const loadCore = useCallback(async (showSpinner = false) => {
    ...
    try {
      const results = await Promise.allSettled([
        fetchJson<BotStatus>("/api/status"), fetchJson<Position[]>("/api/positions"), fetchJson<EquityPoint[]>(`/api/equity?window=${chartWindow.toLowerCase()}&limit=1000`), fetchJson<Performance>("/api/performance"),
...
function filterChartData(equity: EquityPoint[], window: "All" | "24H" | "7D"): Array<EquityPoint & { label: string }> { const timestamps = equity.map((point) => new Date(point.timestamp).getTime()).filter(Number.isFinite); const anchor = timestamps.length ? Math.max(...timestamps) : Date.now(); const cutoff = window === "24H" ? anchor - 86_400_000 : window === "7D" ? anchor - 604_800_000 : Number.NEGATIVE_INFINITY; return equity.filter((point) => new Date(point.timestamp).getTime() >= cutoff).map((point) => { const date = new Date(point.timestamp); return { ...point, label: window === "24H" ? date.toLocaleTimeString([], { hour: "2-digit", minute: "2-digit" }) : date.toLocaleDateString([], { month: "short", day: "numeric" }) }; }); }
function mergeEquity(existing: EquityPoint[], incoming: EquityPoint[]): EquityPoint[] { const byTimestamp = new Map(existing.map((point) => [point.timestamp, point])); incoming.forEach((point) => byTimestamp.set(point.timestamp, point)); return Array.from(byTimestamp.values()).sort((a, b) => new Date(a.timestamp).getTime() - new Date(b.timestamp).getTime()); }
